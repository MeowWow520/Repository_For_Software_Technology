...
bool IsGoingMain = true;
bool IsGoingLast = true;
int x = 1e5, y = 1e5;
int CurrentPlayerType = 1;
int Site[3][3] = { {0,0,0},{0,0,0},{0,0,0} };
int ChackWinCnt = 0, ChackDrawCnt = 0, MessageBoxWinCnt = 0, MessageBoxDrawCnt = 0;
...
int main() {
	...
	while (IsGoingLast) {
		...
		Draw_Tips();
		if (ChackDrawCnt != 0) {
			if (MessageBoxDrawCnt == 0) MessageBox(GetHWnd(), _T("无玩家获胜"), _T("游戏结束"), MB_OK);
			MessageBoxDrawCnt++;
		}
		else if (ChackWinCnt != 0) {
			if (MessageBoxWinCnt == 0) MessageBox(GetHWnd(), _T("玩家获胜"), _T("游戏结束"), MB_OK);
			MessageBoxWinCnt++;
		}
		FlushBatchDraw();
	}
